...
6. **Required tools** — Beyond `github`, which tool prefixes does this persona need? (e.g., `mcp__neon`, `mcp__plugin_playwright_playwright`)
...
Call `mcp__github__list_labels_for_repo` to check if the label exists. If not, create it under the githubclip group via `mcp__github__create_label`.
...
GitHub label "<label>" created under githubclip group.
